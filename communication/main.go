...
func main() {
	...
	c := client.NewClient("http://localhost:3000", "12019861")
	...
	testcaseParams := client.TestCaseParams{
		Stage:    "1",
		Token:    token,
		Testcase: "1",
	}

	_, err = c.GetTestCase(testcaseParams, &testcase)
	if err != nil {
	...
	resp, err := c.SubmitSolution(testcaseParams, solution)
	if err != nil {
		log.Error().
			Err(err).
